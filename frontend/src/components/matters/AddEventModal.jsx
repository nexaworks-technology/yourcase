  ...
  return (
    ...
    >
      <form className="space-y-5">
        <div>
          <label className="text-xs font-semibold uppercase tracking-wide text-slate-500">Event type</label>
            ...
            {eventTypes.map((option) => (
              <button
                ...
                className={`flex items-center gap-2 rounded-2xl border px-3 py-2 text-sm transition ${
                  form.type === option.value ? 'border-blue-200 bg-blue-50 text-blue-600 shadow-sm' : 'border-slate-200 text-slate-600 hover:bg-slate-100'
                }`}
        ...
        <div className="space-y-3">
          ...
          <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-4 py-6 text-center text-sm text-slate-500">
            ...
            <p className="mt-1 text-xs text-slate-400">PDF, DOCX, images up to 25MB</p>
          </div>
          {form.attachments.length > 0 && (
            <ul className="space-y-2 text-sm text-slate-600">
              {form.attachments.map((file, index) => (
                <li key={`${file.name}-${index}`} className="rounded-xl border border-slate-200 bg-white px-3 py-2 shadow-sm">
                  {file.name}
                </li>
              ))}
            </ul>
        ...
        <div className="space-y-3">
          <label className="text-xs font-semibold uppercase tracking-wide text-slate-500">Participants</label>
