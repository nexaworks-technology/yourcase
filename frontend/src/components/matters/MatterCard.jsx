...
export function MatterCard({ matter, onView, onEdit, onArchive }) {
  ...
  return (
    <div
      className="group relative flex h-full flex-col rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-xl"
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="font-mono text-xs tracking-wide text-slate-400">{matterNumber || `MAT-${id?.slice(-4)}`}</p>
          <h3 className="mt-2 text-lg font-semibold text-slate-900">{clientName}</h3>
      ...
      <div className="mt-5 flex items-center justify-between">
        <div className="flex items-center -space-x-2">
          ...
          {assignedLawyers.length === 0 && (
            <div className="flex h-8 items-center gap-1 rounded-full bg-slate-100 px-3 text-xs text-slate-500">
        ...
        <div className="flex items-center gap-2 opacity-0 transition group-hover:opacity-100">
          ...
          <button
            type="button"
            className="rounded-full border border-slate-200 px-3 py-1 text-xs font-medium text-rose-600 hover:bg-rose-50"
            onClick={(event) => {
              event.stopPropagation()
              onArchive?.(matter)
            }}
