  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-300">
            <span>Type</span>
            <select value={type} onChange={(event) => setType(event.target.value)} className="rounded-full border border-slate-200 dark:border-slate-700 px-3 py-1 bg-white dark:bg-slate-900">
      ...
      <div className="space-y-3">
        {filtered.map((query) => (
          <article key={query.id} className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 p-4 shadow-sm">
            <header className="flex flex-wrap items-center justify-between gap-2">
              <div>
                <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-100">{query.title || 'Matter query'}</h3>
                <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500">
                  Asked by {query.user?.name || 'Unknown'} · {new Date(query.createdAt).toLocaleString()}
                </p>
              </div>
              <Badge size="sm" className={typeColors[query.type] || 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300'}>
                {query.type || 'General'}
              </Badge>
            </header>
            <p className="mt-3 text-sm text-slate-600 dark:text-slate-300 line-clamp-3">{query.prompt}</p>
