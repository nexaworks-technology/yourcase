...
export function PageHeader({ title, description, breadcrumbs, actions, className }) {
  return (
    <header className={cn('mb-10 flex flex-col gap-4 border-b border-slate-200/50 pb-6 lg:flex-row lg:items-center lg:justify-between', className)}>
      <div className="space-y-2">
        {breadcrumbs && <nav className="text-sm text-slate-500">{breadcrumbs}</nav>}
        <div>
