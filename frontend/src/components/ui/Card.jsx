...
const variantStyles = {
  ...
  glass: 'bg-white/70 backdrop-blur shadow-lg border border-white/40',
}
...
export function Card({
  ...
  return (
    <div className={baseClass} onClick={onClick} {...props}>
      {loading && (
        <div className="absolute inset-0 z-10 rounded-3xl bg-white/70 backdrop-blur-sm" aria-hidden="true">
      ...
      {header && <div className="flex items-center justify-between gap-3 text-sm text-slate-500">{header}</div>}
...
<Card
  variant="gradient"
  hover
  header={<div className="font-semibold text-slate-700">Case Overview</div>}
