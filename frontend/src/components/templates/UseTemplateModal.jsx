    ...
    >
      <div className="space-y-5">
        <div className="space-y-2">
          <h3 className="text-base font-semibold text-slate-900">Fill in template data</h3>
          <p className="text-sm text-slate-500">Provide the details below and YourCase AI will assemble a polished document tailored to your matter.</p>
        </div>
        ...
        <div className="space-y-4">
          ...
          {variables.map((variable) => {
            ...
            return (
              <div key={variable.name} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                <Component {...baseProps} />
                {variable.type === 'select' && (
                  <p className="mt-2 text-xs text-slate-400">Options: {variable.options?.join(', ')}</p>
                )}
                {variable.helpText && <p className="mt-2 text-xs text-slate-400">{variable.helpText}</p>}
              </div>
            )
          })}
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <p className="mt-2 text-xs text-slate-500">Preview helps validate fields before generating final output.</p>
          <div className="mt-4 space-y-3">
            {Object.entries(values).map(([key, value]) => (
              <div key={key} className="flex items-center justify-between rounded-xl bg-slate-50 px-3 py-2 text-xs text-slate-500">
                <span>{key}</span>
                <span className="max-w-[60%] truncate text-slate-700">{value}</span>
              </div>
            ))}
          </div>
        </div>
